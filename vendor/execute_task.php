...
require_once "./connect.php";
    ...
    if(mysqli_num_rows($result_answer)<1){
        $query_executetask = "UPDATE `tasks` SET `status_img_task`='lose.png', `status_task`=0 WHERE `id`='".$_REQUEST['btn_task']."'";
        mysqli_query($connect,$query_executetask) or die(mysqli_error($connect));
        
    }else{
         $query_executetask = "UPDATE `tasks` SET `status_img_task`='done.png', `status_task`=1 WHERE `id`='".$_REQUEST['btn_task']."'";
        mysqli_query($connect,$query_executetask) or die(mysqli_error($connect));
    }
